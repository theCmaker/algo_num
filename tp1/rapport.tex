...
Votre choix : 
  ...
  \chapter{Conclusion}
    ...
    Les méthodes directes implique des résultats exacts mais l'architecture d'un ordinateur ne peut pas obtenir cette exactitude. On a donc des méthodes directes coûteuses et plus ou moins précises selon le conditionnement du système traité. D'un autre coté, les méthodes itératives permettent des résultats approchés avec un contrôle de précision qu'on ne peut pas avoir avec une méthode directe et ces méthodes sont en général plus rapide.

    Enfin, même si les méthodes itératives sont plus efficaces que les méthodes directes, les 4 méthodes présentées et la sur-relaxation (présentée en annexe) ne sont pas des méthodes adaptées pour des systèmes de grande taille.
    \newpage
  \chapter{Annexe}
